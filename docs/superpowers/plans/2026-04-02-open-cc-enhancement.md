...
**Goal:** Evolve clawnest into a capable AI coding CLI with context management, pluggable tools, sub-agents, memory, skills, and diff view.
...
- `clawnest.py` — Add /memory, /skill, /agents slash commands, diff rendering
...
Run: `cd D:/git/open-cc/clawnest && python -m pytest tests/test_tool_registry.py -v`
...
# tool_registry.py
"""
Tool plugin registry for clawnest.
...
Run: `cd D:/git/open-cc/clawnest && python -c "from tools import execute_tool; from tool_registry import get_all_tools; print(f'{len(get_all_tools())} tools registered'); print(execute_tool('Read', {'file_path': 'config.py'}, 'auto'))"`
...
## Task 3: Diff View (`tools.py` + `clawnest.py`)

**Files:**
- Modify: `tools.py` (the `_edit` and `_write` functions)
- Modify: `clawnest.py` (the `print_tool_end` function)
...
Run: `cd D:/git/open-cc/clawnest && python -m pytest tests/test_diff_view.py -v`
...
- [ ] **Step 6: Add diff rendering to clawnest.py**
...
Also ensure the color dict `C` (line 60-70 of clawnest.py) includes the needed keys. Check and add if missing:
...
Run: `cd D:/git/open-cc/clawnest && python -c "
...
```bash
cd D:/git/open-cc/clawnest
git add tools.py clawnest.py tests/test_diff_view.py
...
Run: `cd D:/git/open-cc/clawnest && python -m pytest tests/test_compaction.py -v`
...
# compaction.py
"""
Context window management for clawnest.
...
Run: `cd D:/git/open-cc/clawnest && python -m pytest tests/test_memory.py -v`
...
# memory.py
"""
File-based memory system for clawnest.
...
MEMORY_DIR = Path.home() / ".clawnest" / "memory"
...
Run: `cd D:/git/open-cc/clawnest && python -m pytest tests/test_subagent.py -v`
...
# subagent.py
"""
Sub-agent lifecycle management for clawnest.
...
- Modify: `clawnest.py` (skill dispatch in REPL)
...
Run: `cd D:/git/open-cc/clawnest && python -m pytest tests/test_skills.py -v`
...
# skills.py
"""
Skill loading and execution for clawnest.
...
SKILL_PATHS = [
    Path.cwd() / ".clawnest" / "skills",
    Path.home() / ".clawnest" / "skills",
]
...
- [ ] **Step 5: Integrate skills into clawnest.py REPL**

In `clawnest.py`, add import near the top:
...
def cmd_skills(_args, _state, _config):
    skills = load_skills()
    if not skills:
        info("No skills found. Place .md files in ~/.clawnest/skills/")
        return True
    for s in skills:
...
```bash
cd D:/git/open-cc/clawnest
git add skills.py tests/test_skills.py clawnest.py
...
- Modify: `clawnest.py` (add /memory, /agents commands)
...
Run: `cd D:/git/open-cc/clawnest && python -m pytest tests/ -v`
...
Run: `cd D:/git/open-cc/clawnest && python -m pytest tests/ -v --tb=short`
