#!/usr/bin/env python3
"""
Generate animated GIF demo of clawnest Telegram Bridge.
...
def draw_phone(img, chat_messages):
    ...
    # Bot name
    d.text((px+52, py+10), "@clawnest_bot", font=FONT_B, fill=(255, 255, 255))
...
def prompt_line(text="", cursor=False):
    cur = "█" if cursor else ""
    return [seg("[clawnest] ", SUBTEXT), seg("» ", CYAN, True), seg(text + cur, TEXT)]
...
def build_scenes():
    ...
    add(BANNER_TG + [
        ok_line("Telegram bridge started (auto). Bot: @clawnest_bot"),
    ...
    add(base + [
        ...
        None,
        ok_line("Telegram bridge is running.  Bot: @clawnest_bot  Chat ID: 123456789"),
        None,
        prompt_line(cursor=True),
    ], 1000, chat=[])
    ...
    phone_init = [
        ("bot", "🟢 clawnest is online. Send me a message and I'll process it.", TEAL),
    ]
    ...
    resp1_lines = [
        ...
        "",
        "  clawnest.py   — Main REPL + slash commands",
    ...
    phone_r1 = phone_q1_typing + [("bot", "Here are the files in this project: clawnest.py, agent.py, tools.py, providers.py, config.py …", GREEN)]
    ...
    add(q3_base + [
        ...
        tool_ok("Found in clawnest.py:480"),
        None,
        tool_line("⚙", "Read", "clawnest.py:480-550", CYAN),
