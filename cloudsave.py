"""
Cloud sync for clawnest sessions via GitHub Gist.
...
Config keys (stored in ~/.clawnest/config.json):
...
def upload_session(
    ...
    filename = f"clawnest_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def list_sessions(token: str, max_results: int = 20) -> tuple[list[dict], str | None]:
    """
    List Gists tagged as clawnest sessions.
